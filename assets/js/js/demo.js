 ...
 */
{
    ...
    class Slide {
        ...
        initEvents() {
            this.mouseenterFn = () => {
                if (!this.isPositionedCenter() || !allowTilt)
                    return;
            ...
            this.mousemoveFn = ev => requestAnimationFrame(() => {
                ...
                if (!allowTilt || !this.isPositionedCenter())
                    return;
                this.tilt(ev);
            });
    ...
    class Slideshow {
        constructor(el) {
            ...
            // The slides.
            this.slides = [];
            Array.from(document.querySelectorAll('.slide')).forEach(slideEl => this.slides.push(new Slide(slideEl)));
        ...
        showContent() {
            if (this.isContentOpen || this.isAnimating)
                return;
            allowTilt = false;
    ...
    //  imagesLoaded( posts, function() {document.querySelectorAll('.slide__img'), {background: true}, () => document.body.classList.remove('loading')});
    /*   imagesLoaded.on( 'done', function( instance ) {
          alert("hello");
        (document.querySelectorAll('.slide__img'), {background: true}, () => document.body.classList.remove('loading'));
      }); */
    //imagesLoaded(alert("Hello"));
